...
def _course_id():
    return next(c["id"] for c in client.get("/api/courses").json() if c["code"] == "CS302")


def test_portable_queries_run_and_postgres_only_queries_are_refused():
    cid = _course_id()
    for q in DEMO_QUERIES:
        resp = client.post(f"/api/dbms/queries/{q['id']}/execute?course_id={cid}")
...
def test_query_catalog_lists_features():
    catalog = client.get("/api/dbms/queries").json()
...
def test_schema_summary_reports_real_row_counts():
    tables = {t["table_name"]: t for t in client.get("/api/dbms/schema").json()}
