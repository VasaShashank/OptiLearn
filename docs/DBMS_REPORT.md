...
artifact that demonstrates it; most can also be shown live on the **DBMS showcase** page.
...
* Migrations: `0001` tables & constraints → `0002` normalization fixes + server-side objects →
  ...
  `0005` co-teaching (`course_members`).
...
23 application relations (plus the `txn_lab_accounts` scratch table used by the Transaction Lab). Core hierarchy: `users 1–1 teachers 1–N courses 1–N units 1–N topics 1–N concepts`.
...
| teacher ↔ course (sharing) | `course_members` | `role` (`co_teacher` / `viewer`), `added_at` |
| concept ↔ assessment (results) | `performance` | score, sample size, errors |

**Constraints (declared, not just validated in Python; counted from `pg_constraint`):** 24 primary keys (one per table), 29 foreign keys with
...
`(teacher_id, code, semester)`), and 41 `CHECK` constraints (value ranges, enumerations such as
`status IN (...)`, `end_date >= start_date`, `concept_id <> prerequisite_id`). Rules that span two tables are triggers instead: a course's owner
cannot also be one of its members (`trg_course_member_not_owner`), and prerequisites must stay
inside one course and acyclic (`trg_prerequisite_guard`).
...
| Triggers (18) | `updated_at` maintenance; **audit** on 11 tables; **weakness flag** from the course threshold; **threshold cascade**; **prerequisite guard** (same course + cycle detection with a recursive CTE, serialised per course with `pg_advisory_xact_lock`); **owner is not a member** on `course_members` |
...
| **Reordering around a UNIQUE constraint** | The curriculum builder saves a new topic order in two phases inside one transaction: every moved row first gets a temporary negative `order_index`, then its final one, so `UNIQUE (unit_id, order_index)` never sees two rows with the same position mid-update. |
| **Carry-over** | Recording a class with *topic not finished* shifts the topics of every later scheduled period by one and deletes their now-mismatched lesson-plan rows in the same transaction as the class record; the matching Mongo documents are deleted after the commit (anything left behind shows up in the consistency check). |
...
| Authorization | Roles `teacher` / `admin`; course-level access (teachers: own courses plus courses shared through `course_members`; other IDs return 404). Co-teachers can edit; viewers get 403 on every write. Nested IDs validated against the URL's course |
...
| SQL console | `READ ONLY` transaction + `SET LOCAL ROLE optiteach_readonly` + **row-level security** (21 policies; child tables defer to parent visibility; shared courses are resolved by `fn_console_course_ids()`, a `SECURITY DEFINER` function, because two policies that read each other's tables make PostgreSQL fail with *infinite recursion detected in policy*) + column privileges hiding `users.hashed_password` + statement timeout + row cap. `set_config()` and `query_to_xml()` are revoked from the console role so it cannot rewrite the settings RLS reads. |
| SQL injection | Parameterized queries throughout (ORM or bound `text()` parameters) |
| Input hardening | Pydantic validation mirroring the CHECK constraints; upload size/type/magic-byte checks and a per-user upload rate limit; HTML-escaped printable exports (stored-XSS fix); explicit CORS origins |
...
| Lesson-plan content, every version, class material | MongoDB `lesson_plan_documents` | Nested (phases, lists, typed resources: slides, video, link, dataset, code, formula), read/written whole, append-only versions; PostgreSQL keeps pointer + status + version |
...
* **CI** (`.github/workflows/ci.yml`): PostgreSQL 17 + MongoDB 8 service containers; migrations
  up/down/up + drift check; seed as `optiteach_app`; Ruff; full test suite; ESLint, typecheck
  and build of the frontend; and a `docker compose` smoke test of the whole stack.
* **Deployment**: `docker compose up` starts PostgreSQL, MongoDB, the API (migrations run on start,
  the sample course is seeded only into an empty database), the web app and Caddy as a single origin.
